...
function initSetHeight() {
	...
	jQuery('.tab-module .tab').each(function() {
		...
		function resizeHandler() {
			...
			tab.css({
				minHeight: winHeight - block.outerHeight() - tabset.outerHeight() - header.outerHeight() - 23
			});
		}
	...
	jQuery('.ui-slider').each(function() {
		...
		slider.slider({
			...
			change: function(event, ui) {
				...
				jQuery(window).trigger('validateForm');
			}
		});
	...
	jQuery('.steps-accordion').steps({
		...
		onInit: function(self) {
			self.stepsBehaviour = {
				...
				step1: {
					init: function(block) {
						var inputs = block.find(self.options.validateRow).find('input:radio, input:checkbox');
						var sliders = block.find(self.options.validateRow + ' .ui-slider');
						...
						jQuery(window).on('validateForm', function() {
		...
		onShow: function(self, slide) {
			...
			setTimeout(function() {
				jQuery(window).trigger('refreshTabs');
			}, 10);
		}
	});
}
...
;(function($) {
	...
	Steps.prototype = {
		...
		validateForm: function(inputs, sliders) {
			var self = this;
		...
		switchTabs: function() {
			...
			switch($nextTab.attr("id")) {
				case "tab-1":
					...
					$('#tab-1').trigger("tabChage1" , [$prevTab.width() , $prevTab.height()]);
					break;
				case "tab-2":
					$('#tab-2').trigger("tabChage2" , [$prevTab.width() , $prevTab.height()]);
					break;
				case "tab-3":
					$('#tab-3').trigger("tabChage3" , [$prevTab.width() , $prevTab.height()]);
					break;
			}
