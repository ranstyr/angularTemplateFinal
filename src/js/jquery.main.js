// page init
//todo ran create directive for jqueryUI componenets
...
function initAccordion() {
	...
	jQuery('.tabset').slideAccordion({
		...
		onInit: function(self) {
			var btnsNext = self.holder.find('.btn-next');
		...
		onShow: function(self, slide) {
			...
			self.items.removeClass(filledClass);
			item.prevAll().addClass(filledClass);

			if (self.items.index(item) >= 3) tabModule.removeClass(disabledClass);
			else tabModule.addClass(disabledClass);
			if (self.items.index(item) >= 2) noteBox.hide();
			else noteBox.show();
			...
			self.items.each(function() {
				...
				if (dataContent && currItem.hasClass(filledClass)) {
	...
	jQuery('.ui-slider').each(function() {
		...
		function refreshText() {
			...
			if (slider.hasClass('money-slider')) {
				eventValue = formatToMoney(Math.max(sliderValue, sliderMin), 0);
				var value = (sliderText ? sliderText : '') + eventValue;
				valueField.val(value);
				if (resultField.length) {
					resultField.text(value);
				}
			} else {
				eventValue = Math.max(sliderValue, sliderMin) + (sliderText ? (' ' + sliderText) : '')
				valueField.val(eventValue);
			}

			labelName = slider.parent().parent().find( 'label:first' )[0]
			...
			slider.trigger("sliderTextChnage" , [eventValue ,labelName ]);
...
;(function($, $win) {
	...
	Tabset.prototype = {
		...
		switchTabs: function() {
			...
			switch($prevTab.attr("id")) {
				case "tab-1":
					$('#tab-1').trigger("tabChage1" , [$prevTab.width()]);
					break;
				case "tab-2":
					$('#tab-2').trigger("tabChage2" , [$prevTab.width()]);
					break;
				case "tab-3":
					$('#tab-3').trigger("tabChage3" , [$prevTab.width()]);
					break;
			}
